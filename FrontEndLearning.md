...
简单来说，每个控件可以当做盒子，它有三个属性，由外向内依次是：外边距(margin)、边框和内边距(padding) ，而内边距的里面才是宽和高。

个人认为理解盒模型还需要先理解 HTML 的布局结构，否则就无法体会到盒模型和 iOS/Android 中坐标模型的区别。HTML 的流式布局结构最大的特点是，默认在同一行排列，而遇到块级元素则换行。

具体的效果和 demo 可以参考这篇文章：[CSS 盒子模型](http://www.runoob.com/try/try.php?filename=trycss_boxmodel)
